...
def listar_recursos_db(db_path: str) -> List[RecursoDigital]:
    init_db(db_path)
    with _connect(db_path) as conn:
        rows = conn.execute("SELECT * FROM recursos ORDER BY id ASC").fetchall()
...
def agregar_recurso_db(db_path: str, recurso: RecursoDigital) -> int:
    init_db(db_path)
    d = recurso.to_dict()
    sql_insert = """
    INSERT INTO recursos (tipo, titulo, autor, anio, isbn, num_paginas, formato, duracion, nivel, episodio, url)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
...
def borrar_recurso_db(db_path: str, id_recurso: int) -> bool:
    init_db(db_path)
    with _connect(db_path) as conn:
...
def existe_recurso_db(db_path: str, id_recurso: int) -> bool:
    init_db(db_path)
    with _connect(db_path) as conn:
        row = conn.execute("SELECT 1 FROM recursos WHERE id = ? LIMIT 1", (id_recurso,)).fetchone()
